...
不同的软件对于覆盖率的指标是不同的，对于要求到的覆盖率类型，应该尽量100%覆盖。结合 Tessy 实践，提高覆盖率的一些建议：

- **按覆盖率类型设计用例**：目标是分支覆盖 100% 时，应基于决策 / 条件组合去设计用例，而不是直接对着代码里的分支写——每条用例要能区分不同的路径走向。
- **先跑一遍，再看报告补用例**：先运行现有用例，用 Tessy 的覆盖率报告（TEP 报告）找出未覆盖的语句 / 分支 / 条件，针对性地补用例，比盲目加用例更高效。
- **用数据驱动批量覆盖**：Tessy 支持通过 Datapool 参数化数据批量驱动用例，覆盖边界值、正常值、异常值，能显著提升条件覆盖和 MC/DC 覆盖。
- **拆分复杂函数**：过大的函数分支组合爆炸、难以覆盖。拆成小函数后用例更聚焦，覆盖率更容易提升，也更好维护。
- **核对不可达代码**：防御性代码、异常分支等可能实际不可达。评估是保留还是移除，避免覆盖率卡在非关键位置。
- **迭代优化并去重**：根据覆盖率报告持续补充用例，同时清理多条覆盖同一路径的冗余用例，保持用例集精简、可追溯。
